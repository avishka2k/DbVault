...
# connection_string = 'Driver={ODBC Driver 17 for SQL Server};Server=tcp:adbms-server.database.windows.net,1433;Database=adbms;Uid=adbms;Pwd=#Apgdb2023;Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30;'

# connection = pyodbc.connect(connection_string)
# cursor = connection.cursor()

# # Create a sample table
# # cursor.execute("IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'sample_table') CREATE TABLE sample_table (id INT IDENTITY(1,1) PRIMARY KEY, name NVARCHAR(255), age INT)")
...
#     name = request.form['field']
#     value = request.form['value']
#     cursor.execute(f"UPDATE sample_table SET {name}=? WHERE id=?", (value, id))
...
#     new_name = request.form['new_name']
#     new_age = request.form['new_age']
#     cursor.execute("INSERT INTO sample_table (name, age) VALUES (?, ?)", (new_name, new_age))
#     return "success"
